...
CREATE TABLE IF NOT EXISTS `students` (
    ...
    `student_name` VARCHAR(64) NOT NULL COMMENT '学生的名字',
    `gender` CHAR(1) NOT NULL DEFAULT 'M' COMMENT '性别 (M/F)',
    `date_of_birth` DATE NOT NULL DEFAULT '1900-01-01' COMMENT '出生日期',
    `address` VARCHAR(255) NOT NULL DEFAULT 'N/A'  COMMENT '地址',
    `phone` VARCHAR(20) NOT NULL DEFAULT 'N/A'  COMMENT '电话号码',
    `email` VARCHAR(128) NOT NULL DEFAULT 'N/A' COMMENT '电子邮箱',
    `enrollment_date` DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '入学日期',
...
CREATE TABLE IF NOT EXISTS `teachers` (
    ...
    `teacher_name` VARCHAR(64) NOT NULL COMMENT '教师的名字',
    `gender` CHAR(1)  NOT NULL DEFAULT 'M' COMMENT '性别 (M/F)',
    `date_of_birth` DATE NOT NULL DEFAULT '1900-01-01' COMMENT '出生日期',
    `address` VARCHAR(255) NOT NULL DEFAULT 'N/A' COMMENT '地址',
    `phone` VARCHAR(20) NOT NULL DEFAULT 'N/A' COMMENT '电话号码',
    `email` VARCHAR(128) NOT NULL DEFAULT 'N/A' COMMENT '电子邮箱',
    `hire_date` DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '聘用日期',
    `is_del` TINYINT DEFAULT '0' COMMENT '标记记录是否被删除，0为未删除，1为已删除',
    `create_time` DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '记录的创建时间',
    `update_time` DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '记录的最后更新时间',
    PRIMARY KEY (`id`) USING BTREE
...
CREATE TABLE IF NOT EXISTS `student_courses` (
    ...
    `course_id` BIGINT UNSIGNED NOT NULL COMMENT '课程的唯一标识符',
    `is_studying` TINYINT DEFAULT '0' COMMENT '标记记录是否正在修读，0为已修读完成，1为正在修读',
    ...
    FOREIGN KEY (`course_id`) REFERENCES `courses`(`id`) ON DELETE CASCADE
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_general_ci COMMENT='学生课程关系表';
...
CREATE TABLE IF NOT EXISTS `exam_results` (
    ...
    `student_id` BIGINT UNSIGNED NOT NULL COMMENT '学生的唯一标识符',
    `score` DECIMAL(5, 2) NOT NULL DEFAULT '0' COMMENT '考试分数',
    `is_finished` TINYINT DEFAULT '0' COMMENT '标记是否已经考完，0为未考完，1为已考完',
    `is_del` TINYINT DEFAULT '0' COMMENT '标记记录是否被删除，0为未删除，1为已删除',
    `create_time` DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '记录的创建时间',
    `update_time` DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '记录的最后更新时间',
    PRIMARY KEY (`id`) USING BTREE,
...
INSERT INTO `students` (`id`, `student_name`, `gender`, `date_of_birth`, `address`, `phone`, `email`, `enrollment_date`, `is_del`, `create_time`, `update_time`) VALUES
(1, '张伟', 'M', '2005-04-23', '北京市朝阳区光华路123号', '13800138000', 'zhangwei@example.com', '2020-09-01', 0, NOW(), NOW()),
(2, '王芳', 'F', '2006-08-15', '上海市徐汇区淮海中路456号', '13900139000', 'wangfang@example.com', '2020-09-01', 0, NOW(), NOW()),
(3, '李子强', 'M', '2004-12-05', '广州市天河区天河路789号', '13700137000', 'liqiang@example.com', '2019-09-01', 0, NOW(), NOW()),
(4, '刘光敏', 'F', '2005-02-20', '深圳市南山区南海大道321号', '13600136000', 'liumin@example.com', '2020-09-01', 0, NOW(), NOW()),
(5, '陈智杰', 'M', '2006-11-30', '成都市武侯区锦绣路654号', '13500135000', 'chenjie@example.com', '2020-09-01', 0, NOW(), NOW()),
(6, '王旭沛', 'F', '2005-07-17', '杭州市西湖区西湖路987号', '13400134000', 'yangyang@example.com', '2020-09-01', 0, NOW(), NOW()),
(7, '赵磊', 'M', '2004-05-12', '武汉市江汉区江汉路543号', '13300133000', 'zhaolei@example.com', '2019-09-01', 0, NOW(), NOW()),
(8, '周静', 'F', '2006-03-03', '南京市鼓楼区鼓楼路876号', '13200132000', 'zhoujing@example.com', '2020-09-01', 0, NOW(), NOW()),
(9, '龙韫', 'M', '2005-09-25', '天津市和平区和平路111号', '13100131000', 'wuqiang@example.com', '2020-09-01', 0, NOW(), NOW()),
(10, '郑丽', 'F', '2006-06-18', '重庆市渝中区解放碑222号', '13000130000', 'zhengli@example.com', '2020-09-01', 0, NOW(), NOW());


INSERT INTO `teachers` (`id`, `teacher_name`, `gender`, `date_of_birth`, `address`, `phone`, `email`, `hire_date`, `is_del`, `create_time`, `update_time`) VALUES
(1, '王伟', 'M', '1980-01-10', '北京市朝阳区光华路123号', '13800138000', 'wangwei@example.com', '2010-09-01', 0, NOW(), NOW()),
(2, '李丽', 'F', '1985-02-15', '上海市徐汇区淮海中路456号', '13900139000', 'lili@example.com', '2012-09-01', 0, NOW(), NOW()),
(3, '张强', 'M', '1975-03-20', '广州市天河区天河路789号', '13700137000', 'zhangqiang@example.com', '2008-09-01', 0, NOW(), NOW()),
(4, '刘敏', 'F', '1982-04-25', '深圳市南山区南海大道321号', '13600136000', 'liumin@example.com', '2011-09-01', 0, NOW(), NOW()),
(5, '陈杰', 'M', '1978-05-30', '成都市武侯区锦绣路654号', '13500135000', 'chenjie@example.com', '2009-09-01', 0, NOW(), NOW()),
(6, '杨飞洋', 'F', '1983-06-05', '杭州市西湖区西湖路987号', '13400134000', 'yangyang@example.com', '2013-09-01', 0, NOW(), NOW()),
(7, '赵以磊', 'M', '1976-07-10', '武汉市江汉区江汉路543号', '13300133000', 'zhaolei@example.com', '2007-09-01', 0, NOW(), NOW()),
(8, '周向静', 'F', '1984-08-15', '南京市鼓楼区鼓楼路876号', '13200132000', 'zhoujing@example.com', '2014-09-01', 0, NOW(), NOW()),
(9, '吴强', 'M', '1979-09-20', '天津市和平区和平路111号', '13100131000', 'wuqiang@example.com', '2010-09-01', 0, NOW(), NOW()),
(10, '张小丽', 'F', '1981-10-25', '重庆市渝中区解放碑222号', '13000130000', 'zhengli@example.com', '2012-09-01', 0, NOW(), NOW());
...
INSERT INTO `student_courses` (`id`, `student_id`, `course_id`, `is_studying`,`date_recorded`, `is_del`, `create_time`, `update_time`) VALUES
(1, 1, 1, 1,'2023-01-10', 0, NOW(), NOW()),
(2, 2, 2, 1, '2023-01-10', 0, NOW(), NOW()),
(3, 3, 3, 1, '2023-01-10', 0, NOW(), NOW()),
(4, 4, 4, 1, '2023-01-10', 0, NOW(), NOW()),
(5, 5, 5, 1, '2023-01-10', 0, NOW(), NOW()),
(6, 6, 6, 1, '2023-01-10', 0, NOW(), NOW()),
(7, 7, 7, 1, '2023-01-10', 0, NOW(), NOW()),
(8, 8, 8, 1, '2023-01-10', 0, NOW(), NOW()),
(9, 9, 9, 0,'2023-01-10', 0, NOW(), NOW()),
(10, 10, 10, 0, '2023-01-10', 0, NOW(), NOW()),
(11, 2, 1, 1,'2023-01-10',0,NOW(), NOW());
...
INSERT INTO `exam_results` (`id`, `exam_id`, `student_id`, `score`,`is_finished`, `is_del`, `create_time`, `update_time`) VALUES
(1, 1, 1, 95.5,1 ,0, NOW(), NOW()),
(2, 2, 2, 88.0,1 , 0, NOW(), NOW()),
(3, 3, 3, 92.0,1, 0, NOW(), NOW()),
(4, 4, 4, 75.0,1, 0, NOW(), NOW()),
(5, 5, 5, 85.0,1 , 0, NOW(), NOW()),
(6, 6, 6, 90.0,1 , 0, NOW(), NOW()),
(7, 7, 7, 78.5,1 , 0, NOW(), NOW()),
(8, 8, 8, 82.0,1 , 0, NOW(), NOW()),
(9, 9, 9, 89.0,1 , 0, NOW(), NOW()),
(10, 10, 10, 87.0,1 , 0, NOW(), NOW()),
(11, 1, 2, 94.0,1 , 0, NOW(), NOW());
